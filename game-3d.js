...
export class Game3D {
    ...
    async init() {
        ...
        this.camera = new THREE.PerspectiveCamera(75, size.width / size.height, 0.1, 220);
    ...
    setupPostProcessing() {
        ...
        const size = this.getViewportSize();
        const strength = this.isMobile ? 0.55 : 0.85;
        const radius = this.isMobile ? 0.24 : 0.35;
        const threshold = 0.85;
        this.bloomPass = new UnrealBloomPass(new THREE.Vector2(size.width, size.height), strength, radius, threshold);
    ...
    setupWorld() {
        ...
        if (this.environment.speedLines) this.scene.add(this.environment.speedLines);
        if (this.environment.innerGlow) this.scene.add(this.environment.innerGlow);
    ...
    reset() {
        this.gameObjects.forEach(obj => this.disposeObject(obj.mesh));
        this.gameObjects = [];
        this.lastSpawnZ = 0;
    ...
    updateGameObjects(delta) {
        for (const obj of this.gameObjects) {
            if (obj.movement) {
                ...
                if (Math.abs(obj.mesh.position.x) > LANE_WIDTH) obj.movement.direction *= -1;
            }
    ...
    checkCollisions() {
        ...
        this.playerColliderCenter.set(this.player.mesh.position.x, this.player.mesh.position.y + 0.65, this.player.mesh.position.z);
