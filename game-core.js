...
/**
 * Hlavní třída hry, která propojuje a řídí všechny moduly.
 */
class GameCore {
    constructor() {
        this.gameState = {};
    ...
    /**
     * Asynchronní inicializace celé hry.
     */
    async init() {
        try {
            await this.threeD.init();
        } catch (error) {
            console.error("Selhání inicializace WebGL/Three.js:", error);
            this.ui.showWebGLFallback();
            return;
        }
    ...
    /**
     * Nastavení všech posluchačů událostí.
     */
    setupEventListeners() {
        ...
        document.addEventListener('keydown', (e) => this.handleInput('keydown', e));
